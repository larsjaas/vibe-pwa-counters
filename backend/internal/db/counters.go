...
// creation timestamp.
func InsertCounter(userID int, name string, step int, initialValue int, cType string, frequency *int64, alertWindow *int64) (*Counter, error) {
	if db == nil {
		return nil, fmt.Errorf("database not initialized")
	}
	const query = `INSERT INTO counters ("user", name, step, initial_value, type, frequency, alert_window, last_performed_at) VALUES ($1, $2, $3, $4, $5, $6, $7, '1970-01-01 00:00:00+00') RETURNING id, createtime, step, initial_value, type, frequency, alert_window`
	var c Counter
	c.UserID = userID
	c.Name = name
	var createTime time.Time
	var insertedStep int
	var insertedInit int
	var insertedType string
	var insertedFreq *int64
	var insertedAlert *int64
	err := db.QueryRow(query, userID, name, step, initialValue, cType, frequency, alertWindow).Scan(&c.ID, &createTime, &insertedStep, &insertedInit, &insertedType, &insertedFreq, &insertedAlert)
	if err != nil {
		return nil, err
	}
	c.CreateTime = createTime
	c.Step = insertedStep
	c.InitialValue = insertedInit
	c.Type = insertedType
...
func GetCountersForUser(userID int) ([]*Counter, error) {
	...
	const query = `
		SELECT DISTINCT c.id, c."user", c.name, c.createtime, c.archivetime, c.step, c.initial_value, c.deletetime, c.type, c.frequency, c.alert_window, c.overdue, c.last_performed_at
		FROM counters c
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.UserID, &c.Name, &c.CreateTime, &archiveTime, &c.Step, &c.InitialValue, &deleteTime, &c.Type, &freq, &alert, &overdue, &lastPerformedAt); err != nil {
			return nil, err
		}
...
func GetCountersByTag(userID int, tagID int) ([]*Counter, error) {
	...
	const query = `
		SELECT DISTINCT c.id, c."user", c.name, c.createtime, c.archivetime, c.step, c.initial_value, c.deletetime, c.overdue
		FROM counters c
	...
	for rows.Next() {
		...
		var step int
		if err := rows.Scan(&c.ID, &c.UserID, &c.Name, &c.CreateTime, &archiveTime, &step, &c.InitialValue, &deleteTime, &overdue); err != nil {
			return nil, err
		}
		if archiveTime.Valid {
			c.ArchiveTime = &archiveTime.Time
