...
// HTTP handler implementations that were originally defined in
// `backend/main.go`. These are now grouped in the `http` internal
// package under `handlers.go` so that the top‑level `main` file can stay
// focused on wiring up routes and database configuration.
